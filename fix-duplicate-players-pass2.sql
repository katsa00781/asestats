...
SELECT
  ...
  (SELECT COUNT(*) FROM deactivate_empty_dupes) AS player_rows_deactivated;

COMMIT;

-- Gyors ellenőrzés ugyanarra a szezonra
WITH normalized AS (
  SELECT
    p.id,
    lower(trim(p.name)) AS name_key,
...
SELECT COUNT(*) AS remaining_within_team_duplicate_groups
FROM within_team;

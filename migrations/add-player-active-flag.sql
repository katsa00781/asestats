-- Játékos státusz (aktív/inaktív) hozzáadása

-- 1. Oszlop hozzáadása
ALTER TABLE players
ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT true;

-- 2. Meglévő rekordok normalizálása
UPDATE players
SET is_active = true
WHERE is_active IS NULL;

-- 3. View-k frissítése: csak aktív játékosok
...
GROUP BY 
  p.id, p.name, p.number, p.position, p.birth_year, p.height, p.weight, p.team_id, p.is_active, s.id, s.name, s.is_current;
